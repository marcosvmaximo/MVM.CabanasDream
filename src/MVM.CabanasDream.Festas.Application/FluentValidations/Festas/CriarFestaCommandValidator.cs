...
public class CriarFestaCommandValidator : AbstractValidator<CriarFestaCommand>
{
    public CriarFestaCommandValidator()
    {
        ...
        RuleFor(command => command.TemaId).NotEqual(Guid.Empty).WithMessage("O ID do tema deve ser fornecido.");
        RuleFor(command => command.ClienteId).NotEmpty().WithMessage("O ID do cliente deve ser fornecido.");
        RuleFor(command => command.ClienteId).NotEqual(Guid.Empty).WithMessage("O ID do cliente deve ser fornecido.");
        RuleFor(command => command.AdministradorId).NotEmpty().WithMessage("O ID do administrador deve ser fornecido.");
        RuleFor(command => command.AdministradorId).NotEqual(Guid.Empty).WithMessage("O ID do administrador deve ser fornecido.");
